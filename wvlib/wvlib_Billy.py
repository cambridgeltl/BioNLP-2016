...
class WVData(object):
    ...
    def words_to_vector(self, words):
        ...
        # this one problematic, used letter as the unit to compute
    ...
    #add Billy 3 Nov 2015, handle ngram word case
    def words_to_vector1(self, words):
        """Return cosine similarity of vectors for given word, which is an N-gram word

        For many invocations of this function, consider calling
        normalize() first to avoid repeatedly normalizing the same
        vectors.
        """
        #check which input word has n-gram
        if ' ' in words:
            totalScore1= numpy.zeros(shape=(200,))
            for word in words.split(' '):
                try:
                    v11 = self.word_to_unit_vector(word)     # use word_to_unit_vector , normalised                   
                except KeyError, e:
                    print 'KeyError - reason "%s"' % str(e)
                    v11 = numpy.zeros(shape=(200,))
                    break
                totalScore1=totalScore1+v11
            v1=totalScore1/len(words.split(' '))     
        else:
            v1 = self.word_to_unit_vector(word)
        return v1
    
    #add Billy 3 Nov 2015, handle ngram word case
    def word_similarity1(self, w1, w2):
        """Return cosine similarity of vectors for given words with n-gram.
        ...
        """
        if ' ' in w1:
            v1=self.words_to_vector1(w1)
        else:
            self.word_to_unit_vector(w1)
                
        if ' ' in w2:
            v2=self.words_to_vector1(w2)
        else:
            v2=self.word_to_unit_vector(w2)
